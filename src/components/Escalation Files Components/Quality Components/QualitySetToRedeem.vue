  ...
  <a-config-provider :locale="enUS">
    ...
    <div class="sticky top-0 z-40 shadow">
      ...
      <div class="bg-white flex justify-center">
        <h2 class="text-2xl font-semibold py-1 mt-2">Set to redeem - Week</h2>
      ...
      <div class="inputs-container bg-white py-3 flex justify-center">
        ...
        <div class="w-2/5 flex justify-around">
          ...
          <a-date-picker
            format="MM-DD-YYYY"
            placeholder="Date "
            v-model="qualityRedeemStore.newRow.date"
            :style="{ width: '250px' }"
          />

          <!-- CINT ID Input -->
          <a-input
            v-model="qualityRedeemStore.newRow.cintID"
            placeholder="CINT ID"
            allow-clear
            :max-length="10"
            :style="{ width: '250px' }"
    ...
    <div>
      ...
      >
        ...
        <tbody v-for="row in qualityRedeemStore.rows" :key="row.id">
          <tr class="border">
            <td data-label="DATE ">
      ...
      <!-- Empty data display -->
      <div class="bg-white mx-auto mt-1 empty_data" v-show="qualityRedeemStore.rows.length === 0">
        <a-empty />
      </div>
      ...
      <div class="absolute bottom-10 z-50 mb-10 ml-10">
        <FloatingMenu
          ...
          @download-data="
            convertTable(
              setRedeemTable,
              `Set to Redeem Week ${storeWeek.week} - ${storeUser.fullName}`
            )
          "
        />
      </div>
...
import { useQualityRedeem } from "../../../stores/Escalation_Files_Stores/Quality_Stores/qualityRedeem";
...
const qualityRedeemStore = useQualityRedeem();
...
function clearInputFields() {
  qualityRedeemStore.newRow.date = "";
  qualityRedeemStore.newRow.cintID = "";
}

// Function to add new row to the table
function addNewRow() {
  qualityRedeemStore.$patch((state) => {
