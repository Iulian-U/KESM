<template>
  <div class="w-full h-screen flex justify-center items-center registerBackground">
    <!-- Display loading icon if auth is pending status is true -->
    <div
      class="z-40 absolute bg-indigo-500 w-full h-screen opacity-60 flex items-center justify-center"
      v-show="isPending"
    >
      <div>
        <Loader />
    ...
    <div class="bg-white p-16 rounded-sm shadow-md">
      ...
      <!-- Form title -->
      <h2 class="text-indigo-500 text-xl font-semibold">Register Account</h2>
      ...
      <a-space direction="vertical" size="large" :style="{ width: '350px' }">
        ...
        <a-form :model="form" :layout="layout">
          ...
          <!-- Form submit button -->
          <a-form-item>
            <a-button type="outline" @click="handleSubmit()">Register</a-button>
          </a-form-item>
        </a-form>
      </a-space>
      ...
      <div class="my-2 text-gray-500">
        <div class="flex italic">
          ...
          <div class="hover:text-indigo-700 ml-1">
...
//Function to register the user using useSignup composable
const registerUser = async () => {
  if (form.firstName != "" && form.lastName != "" && form.email != "" && form.password != "") {
    ...
    error.value = "Please make sure to fill out all the fields";
  }
};

//Function to handle the submit button
const handleSubmit = () => {
  if (!form.email.includes("@iulian-ursu.com") && form.email.length > 2) {
    error.value =
      "The email address entered is not part of the Project this App has been assigned to";
  } else {
    registerUser();
  }
};
